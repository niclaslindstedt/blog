...
| `summary`   | single line                 | yes      | One-sentence lede shown in the list view as the clickable preview — the terminal renders it via `grep "^summary:" *.md` after `ls -1`, and the prose fallback renders it under the title. Keep it on a single line, no line breaks.                                                                                      |
