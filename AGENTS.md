...
- **Frontmatter schema**: posts carry `title`, `date`, `edited_at`, `summary`, and an optional comma-separated `tags` — independently per audience version. `date` and `edited_at` are ISO 8601 UTC datetimes (`YYYY-MM-DDTHH:MM:SSZ`, `Z` required). `summary` is a single-line lede shown in the terminal's `grep -oP '(?<=^summary: ).*' *.md` preview and in the prose fallback under the title. The title is in frontmatter, not as a `#` heading in the body.
